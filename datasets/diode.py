...
class DIODE(Dataset):
    ...
    def get_sample_with_angle(self, item, rotate_angle, force_mask=False):
        ...
        mask_path = os.path.join(self.data_root, self.filelist[item].split(' ')[2])
        ...
        depth = self._read_npy_file(depth_path).astype('float32')
        mask = self._read_npy_file(mask_path).astype('bool')

        sample = self.transform({'image': image, 'depth': depth, 'mask': mask})
        sample['image'] = torch.from_numpy(sample['image'])
        if self.need_mask or force_mask:
            sample['depth'] = torch.from_numpy(sample['depth'])
            sample['mask'] = (torch.from_numpy(sample['mask']) > 0.5)  & (sample['depth'] > self.min_depth) & (sample['depth'] < self.max_depth)

...
ckpt_path = "./checkpoints/large/model.ckpt"

image_path = f"/home/couser/datasets/open_scene/visualization_samples/{sample_id}.jpg" #"/home/couser/datasets/open_scene/image/frame_00001.jpg"
lidar_path = f"/home/couser/datasets/open_scene/visualization_samples/{sample_id}.npy"

size = (560, 560)
keep_aspect_ratio = False
model_name = "promptda" #"dav2" #"distill" #"rotate_pda" # "promptda"

os.makedirs(f"./visualization_{sample_id}_{model_name}", exist_ok=True)
...
def get_image(rotate_angle, ratio, keep_aspect_ratio):
    image = load_image(image_path)[0] # B, C, H, W
...
model = PromptDA.from_pretrained(ckpt_path).cuda().eval()
...
def get_input_image(rotate_angle):
    ...
    depth = load_depth(lidar_path)
    #print(depth.shape)
    depth = F.interpolate(depth, (orig_shape[1] // 4, orig_shape[2] // 4), mode='bilinear', align_corners=True)[0]
    #depth = depth.unsqueeze(0)
    #depth = tensor_rotate(depth, 90)
    
    # rotate (padding only) and resize
    image = tensor_rotate(image, rotate_angle)
    depth = tensor_rotate(depth, rotate_angle)

    #rotate_image = crop_with_ratio(rotate_image, ratio=ratio, keep_aspect_ratio=keep_aspect_ratio)
    _, height, width = image.shape
    image = tensor_resize_to_14multiple(image)
    
    return image, depth, height, width, orig_shape

def get_depth(rotate_angle, ratio, keep_aspect_ratio, ground_truth=None, return_org=False, display_angle=None):
    image, prompt_depth, height, width, orig_shape = get_input_image(rotate_angle)
...
for rotate_angle, ratio in ((0, 0.9), (-10, 0.7), (-15, 0.6), (-30, 0.45), (-45, 0.43), (-75, 0.6), (-90, 0.8)):
    ...
    result_depth.save(f"./visualization_{sample_id}_{model_name}/{model_name}_angle{int(-rotate_angle)}_ratio{int(ratio * 100)}_depth.jpg")

    image = get_image(rotate_angle, ratio, keep_aspect_ratio)
    result_image = visualize_image(image)
    result_image.save(f"./visualization_{sample_id}_{model_name}/{model_name}_angle{int(-rotate_angle)}_ratio{int(ratio * 100)}.jpg")
    ...
    result_diff.save(f"./visualization_{sample_id}_{model_name}/{model_name}_angle{int(-rotate_angle)}_ratio{int(ratio * 100)}_diff.jpg")

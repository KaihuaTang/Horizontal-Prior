...
parser = argparse.ArgumentParser(description='Depth Anything V2 Evaluation')
...
def main():
    ...
    for rotate_range in ((0, 0), (0, 15), (0, 45), (0, 90)):
        ...
        for dataset_name in dataset_list:
            ...
            all_results, all_nsamples = single_dataset_evaluation(args, dataset_name, testloader, model, logger, rotate_range, all_results, all_nsamples, per_sample_records)
...
def single_dataset_evaluation(args, dataset_name, testloader, model, logger, rotate_range, all_results, all_nsamples, per_sample_records=None):
    ...
    for i, sample in tqdm(enumerate(testloader)):
        ...
        if 'rotate_angle' in sample:
            ra = sample['rotate_angle']
            gt_rotate = float(ra.item()) if torch.is_tensor(ra) and ra.numel() == 1 else float(ra[0])
        else:
            gt_rotate = 0.0
        if 'item' in sample:
            it = sample['item']
            item_id = int(it[0].item()) if torch.is_tensor(it) else int(it[0])
        else:
            item_id = i
        record = {
